...
class DataExportApplet(Applet):
    ...
    def __init__(self, workflow, title, isBatch=False):
        ...
        if self.topLevelOperator is None:
            self.__topLevelOperator = OpMultiLaneWrapper(
                OpDataExport, parent=workflow, promotedSlotNames=set(["RawData", "Inputs", "RawDatasetInfo"])
            )
    ...
    def make_cmdline_parser(cls, starting_parser=None):
        ...
        arg_parser.add_argument(
            "--output_axis_order", help="Axis indexing order of exported data, e.g. tzyxc", required=False
        )
        arg_parser.add_argument("--output_format", help="Export file format", choices=all_format_names, required=False)
        arg_parser.add_argument(
            "--output_filename_format",
            help="Output file path, including special placeholders, e.g. /tmp/results_t{t_start}-t{t_stop}.h5",
            required=False,
        )
        arg_parser.add_argument(
            "--output_internal_path",
            help="Specifies dataset name within an hdf5 dataset (applies to hdf5 output only), e.g. /volume/data",
            required=False,
        )

        arg_parser.add_argument(
            "--export_source",
            help="The data to export.  See the dropdown list on the Data Export page for choices.",
            required=False,
        )
    ...
    def parse_known_cmdline_args(cls, cmdline_args, parsed_args=None):
        ...
        if parsed_args.export_drange:
            if not parsed_args.pipeline_result_drange:
                raise Exception(
                    "Cannot renormalize to custom export_drange without a specified pipeline_result_drange."
                )
    ...
    def _configure_operator_with_parsed_args(cls, parsed_args, opDataExport):
        ...
        if parsed_args.export_source is not None:
            ...
            try:
                ...
                raise Exception(
                    "Invalid option for --export_source: '{}'\n"

...
from ilastik.applets.brushing.brushingImport import import_brushing_layer
...
class BrushingGui(LayerViewerGui):
    ...
    applet with the added functionality of brushing.
    """
    ###########################################
    ...
    def stopAndCleanUp(self):
        super(BrushingGui, self).stopAndCleanUp()
    ...
    class BrushingSlots(object):
        """
        This class serves as the parameter for the BrushingGui constructor.
        It provides the slots that the brushing GUI uses to source labels to the display and sink labels from the
        user's mouse clicks.
        """
        def __init__(self):
    ...
    def __init__(self, parentApplet, brushingSlots, topLevelOperatorView, drawerUiPath=None, rawInputSlot=None, crosshair=True):
        """
        Constructor.

        :param brushingSlots: Provides the slots needed for sourcing/sinking label data.  See BrushingGui.BrushingSlots
        ...
        assert isinstance(brushingSlots, BrushingGui.BrushingSlots)
        assert brushingSlots.labelInput is not None, "Missing a required slot."
        assert brushingSlots.labelOutput is not None, "Missing a required slot."
        assert brushingSlots.labelEraserValue is not None, "Missing a required slot."
        assert brushingSlots.labelDelete is not None, "Missing a required slot."
        assert brushingSlots.labelNames is not None, "Missing a required slot."

        self.__cleanup_fns = []

        self._brushingSlots = brushingSlots
        ...
        if drawerUiPath is None:
            # Default ui file
            drawerUiPath = os.path.split(__file__)[0] + '/brushingDrawer.ui'
        self._initLabelUic(drawerUiPath)

        # Init base class
        super(BrushingGui, self).__init__(parentApplet,
                                          topLevelOperatorView,
                                          [brushingSlots.labelInput, brushingSlots.labelOutput],
                                          crosshair=crosshair)

        self.__initShortcuts()
        self._brushingSlots.labelEraserValue.setValue(self.editor.brushingModel.erasingNumber)
    ...
    def _initLabelUic(self, drawerUiPath):
        ...
        def handleLabelMergeRequested(from_row, from_name, into_row, into_name):
            ...
            names = list(self._brushingSlots.labelNames.value)
            names.pop(from_label-1)
            self._brushingSlots.labelNames.setValue( names )
        '''
        ...
        self.paintBrushSizeIndex = PreferencesManager().get( 'brushing', 'paint brush size', default=0 )
        self.eraserSizeIndex = PreferencesManager().get( 'brushing', 'eraser brush size', default=4 )
    ...
    def __initShortcuts(self):
        ...
        shortcutGroupName = "Brushing"

        '''
        if hasattr(self.brushingDrawerUi, "AddLabelButton"):

            mgr.register("a", ActionInfo( shortcutGroupName,
                                          "New Label",
                                          "Add New Label Class",
                                          self.brushingDrawerUi.AddLabelButton.click,
                                          self.brushingDrawerUi.AddLabelButton,
                                          self.brushingDrawerUi.AddLabelButton ) )
        '''
        ...
        mgr.register( "n", ActionInfo( shortcutGroupName,
                                       "Navigation Cursor",
                                       "Navigation Cursor",
                                       self.brushingDrawerUi.arrowToolButton.click,
                                       self.brushingDrawerUi.arrowToolButton,
                                       self.brushingDrawerUi.arrowToolButton ) )

        mgr.register( "b", ActionInfo( shortcutGroupName,
                                       "Brush Cursor",
                                       "Brush Cursor",
                                       self.brushingDrawerUi.paintToolButton.click,
                                       self.brushingDrawerUi.paintToolButton,
                                       self.brushingDrawerUi.paintToolButton ) )

        mgr.register( "e", ActionInfo( shortcutGroupName,
                                       "Eraser Cursor",
                                       "Eraser Cursor",
                                       self.brushingDrawerUi.eraserToolButton.click,
                                       self.brushingDrawerUi.eraserToolButton,
                                       self.brushingDrawerUi.eraserToolButton ) )
        ...
        mgr.register( ",", ActionInfo( shortcutGroupName,
                                       ...
                                       self.brushingDrawerUi.brushSizeComboBox,
                                       self.brushingDrawerUi.brushSizeComboBox ) )
        ...
        '''
        if hasattr(self.brushingDrawerUi, "thresToolButton"):
            mgr.register( "t", ActionInfo( shortcutGroupName,
                                           ...
                                           self.brushingDrawerUi.thresToolButton.click,
                                           self.brushingDrawerUi.thresToolButton,
                                           self.brushingDrawerUi.thresToolButton ) )
        '''
    ...
    def _updateLabelShortcuts(self):
        ...
        for i in range(numShortcuts,numRows):
            ...
            action_info = ActionInfo( "Brushing", 
    ...
    def hideEvent(self, event):
        ...
        super(BrushingGui, self).hideEvent(event)
    ...
    def _gui_enableBrushing(self, enable):
    ...
    def _updateLabelList(self):
        ...
        names = self._brushingSlots.labelNames.value
        numLabels = len(self._brushingSlots.labelNames.value)
    ...
    def _onLabelRemoved(self, parent, start, end):
        ...
        if len(self._brushingSlots.labelNames.value) > self._labelControlUi.labelListModel.rowCount():
            ...
            self._brushingSlots.labelDelete.setValue(row+1)
            ...
            self._brushingSlots.labelDelete.setValue(-1)
            
            labelNames = self._brushingSlots.labelNames.value
            labelNames.pop(start)
            self._brushingSlots.labelNames.setValue(labelNames, check_changed=False)
    '''
    ...
    def createLabelLayer(self, direct=False):
        ...
        """
        labelOutput = self._brushingSlots.labelOutput
    ...
    def allowDeleteLastLabelOnly(self, enabled):
        """
        In the TrackingWorkflow when brushing 0/1/2/.../N mergers we do not allow

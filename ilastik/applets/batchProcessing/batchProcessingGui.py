...
from ilastik.applets.dataSelection.dataSelectionGui import (
    DataSelectionGui,
)  # We borrow the file selection window function.
...
class BatchProcessingGui(QTabWidget):
    ...
    def initMainUi(self):
        ...
        for role_index, role_name in enumerate(role_names):
            select_button = QPushButton(
                "Select " + role_name + " Files...", clicked=partial(self.select_files, role_index)
            )
            clear_button = QPushButton("Clear " + role_name + " Files", clicked=partial(self.clear_files, role_index))
    ...
    def run_export(self):
        ...
        for role_index, list_widget in enumerate(self.list_widgets[1:], start=1):
            ...
            assert len(role_path_dict[role_index]) <= num_datasets, "Too many files given for role: '{}'".format(
                role_names[role_index]
            )

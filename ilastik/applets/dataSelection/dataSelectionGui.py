...
from PyQt5.QtWidgets import QDialog, QFileDialog, QMessageBox, QStackedWidget, QWidget
...
class DataSelectionGui(QWidget):
            ...
            if len(fileNames) > max_files:
                msg = (
                    ...
                    "in the context menu or the button in the last row.".format(
                        num_selected=len(fileNames), num_slots=max_files
                    )
                )
    ...
    def repairDatasetInfo(self, info, roleIndex, laneIndex):
        ...
        editorDlg = DatasetInfoEditorWidget(
            self, self.topLevelOperator, roleIndex, [laneIndex], defaultInfos, show_axis_details=self.show_axis_details
        )
    ...
    def editDatasetInfo(self, roleIndex, laneIndexes):
        editorDlg = DatasetInfoEditorWidget(
            self, self.topLevelOperator, roleIndex, laneIndexes, show_axis_details=self.show_axis_details
        )
        editorDlg.exec_()
        self.parentApplet.appletStateUpdateRequested()
    ...
    def updateInternalPathVisiblity(self):
        for view in self._detailViewerWidgets:
            model = view.model()
            view.setColumnHidden(DatasetDetailedInfoColumn.InternalID, not model.hasInternalPaths())

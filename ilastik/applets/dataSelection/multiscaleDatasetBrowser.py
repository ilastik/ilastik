...
class CheckRemoteStoreWorker(QThread):
    ...
    def __init__(self, parent, store_class, *args, **kwargs):
        super().__init__(parent)
        self.store_class = store_class
        self.args = args  # store classes have different interfaces
        self.kwargs = kwargs

    def run(self):
        try:
            store = self.store_class(*self.args, **self.kwargs)
...
class MultiscaleDatasetBrowser(QDialog):
    ...
    def _validate_text_input(self, _event):
        ...
        if OMEZarrStore.is_uri_compatible(uri):
            ...
            worker = CheckRemoteStoreWorker(self, OMEZarrStore, uri)
        elif RESTfulPrecomputedChunkedVolume.is_uri_compatible(uri):
            StoreTypeMatchingUri = RESTfulPrecomputedChunkedVolume
            worker = CheckRemoteStoreWorker(self, RESTfulPrecomputedChunkedVolume, volume_url=uri)
        else:

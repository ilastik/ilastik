    ...
    def paint(self, painter, option, index):
        ...
        if index.row() < parent_view.model().rowCount() - 1 and parent_view.model().isEmptyRow(index.row()):
            if button is None:
                ...
                button.addFilesRequested.connect(partial(parent_view.handleCellAddFilesEvent, index.row()))
                button.addStackRequested.connect(partial(parent_view.handleCellAddStackEvent, index.row()))

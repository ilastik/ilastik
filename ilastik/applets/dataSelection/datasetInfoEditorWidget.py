...
class DatasetInfoEditorWidget(QDialog):
    ...
    def _setUpEventFilters(self):
        ...
        self._autoAppliedWidgets = {
            self.nicknameEdit: self._applyNicknameToTempOps,
    ...
    def _getClosingMessages(self):
        ...
        if need_message_about_storage:
            msg = (
                "Your dataset will be copied to your project file when your project is saved.\n"
    ...
    def _applyNormalizeDisplayToTempOps(self):
        ...
        if new_norm is False and (
            new_drange[0] == self.rangeMinSpinBox.minimum() or new_drange[1] == self.rangeMaxSpinBox.minimum()
        ):
            # no drange given, autonormalization cannot be switched off !
            QMessageBox.warning(
                None, "Warning", "Normalization cannot be switched off without specifying the data range !"
            )
            self.normalizeDisplayComboBox.setCurrentIndex(1)
            return
    ...
    def _applyRangeToTempOps(self):
        ...
        if new_drange[0] == self.rangeMinSpinBox.minimum() or new_drange[1] == self.rangeMaxSpinBox.minimum():
            new_drange = None
        ...
        try:
            ...
            if new_drange is not None:
                if new_drange[0] >= new_drange[1]:
                    QMessageBox.warning(
                        self, "Error", "Can't apply data range values: Data range MAX must be greater than MIN."
                    )
                ...
                for laneIndex, op in list(self.tempOps.items()):
                    ...
                    if new_drange[0] < dtype_info.min or new_drange[1] > dtype_info.max:
                        QMessageBox.warning(
                            ...
                            "(Full range of {} is [{}, {}].)".format(
                                new_drange, laneIndex, dtype_info.dtype.name, dtype_info.min, dtype_info.max
                            ),
                        )
                        self._error_fields.add("Data Range")
                        return False

...
from lazyflow.operators.ioOperators.opStreamingHdf5SequenceReaderM import OpStreamingHdf5SequenceReaderM
from lazyflow.operators.ioOperators.opStreamingHdf5SequenceReaderS import OpStreamingHdf5SequenceReaderS
...
class DataSelectionSerializer(AppletSerializer):
    ...
    # Constants
    LocationStrings = {
        DatasetInfo.Location.FileSystem: u"FileSystem",
    ...
    def _serializeToHdf5(self, topGroup, hdf5File, projectFilePath):
        ...
        for laneIndex, multislot in enumerate(self.topLevelOperator.DatasetGroup):
            for roleIndex, slot in enumerate(multislot):
                ...
                if info.location == DatasetInfo.Location.ProjectInternal and info.datasetId not in list(
                    localDataGroup.keys()
                ):
                    ...
                    try:
                        opWriter = OpH5WriterBigDataset(
                            parent=self.topLevelOperator.parent, graph=self.topLevelOperator.graph
                        )
                        opWriter.CompressionEnabled.setValue(
                            False
                        )  # Compression slows down browsing a lot, and raw data tends to be noisy and doesn't compress very well, anyway.
    ...
    def importStackAsLocalDataset(self, info, sequence_axis="t"):
        ...
        if firstPathParts.extension.lower() in OpTiffReader.TIFF_EXTS:
            ...
            assert not (isMultiFile and isSingleFile), "Something is wrong, glob string shouldn't allow both"
            assert isMultiFile or isSingleFile, "Glob string doesn't conform to h5 stack glob string rules"
    ...
    def _readDatasetInfo(self, infoGroup, localDataGroup, projectFilePath, headless):
        ...
        try:
            ...
            datasetInfo.fromstack = datasetInfo.location == DatasetInfo.Location.ProjectInternal and (
                ("?" in datasetInfo._filePath) or (os.path.pathsep in datasetInfo._filePath)
            )
            ...
            if not os.path.exists(filePath):
                if headless:
                    if self._shouldRetrain:
                        raise RuntimeError(
                            "Retrain was passed in headless mode, " "but could not find data at " + filePath
                        )
                    else:
                        assert (
                            datasetInfo.laneShape

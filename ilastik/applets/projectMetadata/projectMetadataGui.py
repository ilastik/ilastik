...
class ProjectMetadataGui( QWidget ):
    ...
    def initMainUi(self):
        ...
        def handleProjectNameWidgetChanged(newText):
            self._projectMetadata.projectName = newText.encode('utf-8')
        ...
        def handleLabelerWidgetChanged(newText):
            self._projectMetadata.labeler = newText.encode('utf-8')
        ...
        def handleDescriptionWidgetChanged():
            self._projectMetadata.description = self.descriptionEdit.toPlainText().encode('utf-8')
        ...
        def handleMetadataChanged():
            if self.projectNameEdit.text() != self._projectMetadata.projectName:
                self.projectNameEdit.setText( self._projectMetadata.projectName.decode('utf-8') )
            if self.labelerEdit.text() != self._projectMetadata.labeler:
                self.labelerEdit.setText( self._projectMetadata.labeler.decode('utf-8') )
            if self.descriptionEdit.toPlainText() != self._projectMetadata.description:
                self.descriptionEdit.setText( self._projectMetadata.description.decode('utf-8') )

...
from qtpy.QtGui import QColor, QIcon, QPixmap, QTextCursor
...
class AnnotationsGui(LayerViewerGui):
    ...
    def goToNextUnlabeledDivision(self):
        self.divFeatures = self.topLevelOperatorView.DivisionProbabilities(
            list(range(0, self.topLevelOperatorView.LabelImage.meta.shape[0]))
        ).wait()  # , {'RegionCenter','Coord<Minimum>','Coord<Maximum>'}).wait()
        ...
        for t in range(time_start, time_stop):
            stop = list(shape)
            stop[0] = t + 1
            roi = SubRegion(self.topLevelOperatorView.LabelImage, start=[t, 0, 0, 0, 0], stop=stop)
            ...
            for ul in uniqueLabels:
                ...
                if (
                    not divFlag
                    and ul > 0
                    and t in range(len(self.divFeatures))
                    and ul in range(len(self.divFeatures[t]))
                    and self.divFeatures[t][ul][1] > self.divisionProbabilityCutOff
                ):
                    divisionCandidates.append([t, ul, self.divFeatures[t][ul][1]])
        if divisionCandidates == []:
            self._informationMessage(
                "No more DIVISIONS found for the division probability cut-off {}!".format(
                    self.divisionProbabilityCutOff
                )
            )
        else:
                ...
                if idx == ul:
                    ...
                    if len(rc[idx]) == 2:
                        ...
                        raise DatasetConstraintError(
                            "Tracking", "The RegionCenter feature must have dimensionality 2 or 3."
                        )
    ...
    def goToNextUnlabeledMerger(self):
        self.mergerFeatures = self.topLevelOperatorView.DetectionProbabilities(
            list(range(0, self.topLevelOperatorView.LabelImage.meta.shape[0]))
        ).wait()  # , {'RegionCenter','Coord<Minimum>','Coord<Maximum>'}).wait()
            ...
            for ul in uniqueLabels:
                if ul > 0:
                    ...
                    if maxValue > self.mergerProbabilityCutOff:
                        mergerIndex = index
                    if mergerIndex > 0:
                        if (t not in labels.keys() or not ul in labels[t].keys()) or (
                            ul in labels[t].keys() and mergerIndex > len(labels[t][ul])
                        ):
                            mergerCandidates.append([t, ul, maxValue])

        if mergerCandidates == []:
            self._informationMessage(
                "No more MERGERS found for the merger probability cut-off {}!".format(self.mergerProbabilityCutOff)
            )
        else:
    ...
    def __init__(self, parentApplet, topLevelOperatorView):
        ...
        self.features = self.topLevelOperatorView.ObjectFeatures(
            list(range(0, self.topLevelOperatorView.LabelImage.meta.shape[0]))
        ).wait()  # , {'RegionCenter','Coord<Minimum>','Coord<Maximum>'}).wait()
    ...
    def _onInitializeAnnotations(self):
        self._questionMessage(
            "All your annotations will be lost! You should save the project, "
        ...
        if division:
            ...
            for parentTrack in self.topLevelOperatorView.divisions.keys():
                ...
                if not (parent and child1 and child2):
                    logger.info(
                        "WARNING:Your divisions and labels do not match for time {} and parent track {} with label {}!".format(
                            t, parentTrack, parent
                        )
                    )
                    pass
                else:
            ...
            if time is not None and label is not None and track is not None:
                ...
                self.topLevelOperatorView.Annotations.value["labels"][time][label] = self.topLevelOperatorView.labels[
                    time
                ][label]
                ...
                and time_interval[0] is not None
                and time_interval[1] is not None
                ...
                for t in time_range:
                    ...
                    self.topLevelOperatorView.Annotations.value["labels"][t][label] = self.topLevelOperatorView.labels[
                        t
                    ][label]
            else:
                ...
                if (
                    ...
                    and self.topLevelOperatorView.Annotations.value["divisions"] == {}
                    and "labels" in self.topLevelOperatorView.Annotations.value.keys()
                    and self.topLevelOperatorView.Annotations.value["labels"] == {}
                ):
                ...
                for t in self.topLevelOperatorView.labels.keys():
                    for lab in self.topLevelOperatorView.labels[t].keys():
                        if (
                            t not in self.topLevelOperatorView.Annotations.value["labels"].keys()
                            and lab in self.topLevelOperatorView.Annotations.value["labels"][t].keys()
                        ):
    ...
    def _setDivisionsList(self):
        ...
        for trackid in list(self.mainOperator.divisions.keys()):
            self._addDivisionToListWidget(
                trackid,
                self.mainOperator.divisions[trackid][0][0],
                self.mainOperator.divisions[trackid][0][1],
    ...
    def _setDirty(self, slot, timesteps):
        if slot is self.mainOperator.TrackImage:
            roi = SubRegion(
                self.mainOperator.TrackImage,
                start=[min(timesteps)] + 4 * [0],
                stop=[max(timesteps) + 1] + list(self.mainOperator.TrackImage.meta.shape[1:]),
            )
            ...
            roi = SubRegion(
                self.mainOperator.UntrackedImage,
                start=[min(timesteps)] + 4 * [0],
                stop=[max(timesteps) + 1] + list(self.mainOperator.TrackImage.meta.shape[1:]),
            )
        ...
        if self.divLock:
            ...
            if not oid == 0:
                ...
                if len(self.divs) == 0:
                    ...
                    if position5d[0] != self.divs[0][0] + 1:
                        self._criticalMessage(
                            "Error: The daughter cells are expected to be in time step " + str(self.divs[0][0] + 1)
                        )
                        return
                ...
                if len(self.divs) == 3:
                    ...
                    if (self.divs[0][1] not in self.mainOperator.labels[self.divs[0][0]]) or (
                        activeTrack not in self.mainOperator.labels[self.divs[0][0]][self.divs[0][1]]
                    ):
                    ...
                    self._log(
                        "division (t,parent,child1,child2) = "
                        + str((self.editor.posModel.time - 1, div[0], div[1], div[2]))
                        + " added."
                    )
            ...
            if res == -99 or res == -98:
                self._informationMessage(
                    "Info: Object " + str(oid) + " in time frame " + str(t) + " is outside the current crop."
                )
                return
            elif res == -1:
                return
    ...
    def handleEditorRightClick(self, position5d, globalWindowCoordiante):
        ...
        for l in trackids:
            ...
            if isAppearance:
                ...
                opTracking._type(t, l)[0] in ["FIRST", "SINGLETON(FIRST_LAST)"] and t > 0 and l not in divisionChildren
            ):
            ...
            if isDisappearance:
                ...
                opTracking._type(t, l)[0] in ["LAST", "SINGLETON(FIRST_LAST)"]
                and t < maxTime - 1
                and l not in delDivision.values()
            ):
    ...
    def handleEditorToolTip(self, position5d, globalWindowCoordiante):
        ...
        if self.divLock:
            if len(self.divs) == 0:
                self.setToolTip(
                    "Left click on a parent, then left click on each of the children to annotate a division."
                )
            ...
            self.setToolTip(
                "Left click to add the active track to this unlabelled object. Right click for more options."
            )
        else:
    ...
    def _delLabel(self, t, oid, track2remove, errorMessage=True):
        if t in list(self.labelsWithDivisions.keys()) and track2remove in self.labelsWithDivisions[t] and errorMessage:
            self._criticalMessage(
                "Error: Cannot remove label "
                + str(track2remove)
                + " at t="
                + str(t)
                + ", since it is involved in a division event."
    ...
    def _removeAppearance(self, t, oid, track):
        if (
            t in self.mainOperator.appearances.keys()
            and oid in self.mainOperator.appearances[t].keys()
            and track in self.mainOperator.appearances[t][oid].keys()
        ):
    ...
    def _removeDisappearance(self, t, oid, track):
        if (
            t in self.mainOperator.disappearances.keys()
            and oid in self.mainOperator.disappearances[t].keys()
            and track in self.mainOperator.disappearances[t][oid].keys()
        ):
    ...
    def _isAppearance(self, time, oid, label):
        if (
            time in self.mainOperator.appearances.keys()
            and oid in self.mainOperator.appearances[time].keys()
            and label in self.mainOperator.appearances[time][oid].keys()
        ):
    ...
    def _isDisappearance(self, time, oid, label):
        if (
            time in self.mainOperator.disappearances.keys()
            and oid in self.mainOperator.disappearances[time].keys()
            and label in self.mainOperator.disappearances[time][oid].keys()
        ):
    ...
    def _addObjectToTrack(self, activeTrack, oid, t):
        ...
        if activeTrack == self.misdetIdx:
            if len(self.mainOperator.labels[t][oid]) > 0:
                if self.misdetIdx not in self.mainOperator.labels[t][oid]:
                    self._informationMessage(
                        "Info: This object is already marked as part of a track, cannot mark it as a misdetection."
                    )
            ...
            for tracklist in list(self.mainOperator.labels[t].values()):
                if activeTrack in tracklist:
                    if activeTrack not in self.mainOperator.labels[t][oid]:
                        self._informationMessage(
                            "Info: There is already an object with this track id in this time step."
                        )
                        return -1
                    elif t == maxTime:
        ...
        if self.misdetIdx in self.mainOperator.labels[t][oid]:
            self._criticalMessage(
                "Error: This object is already marked as a misdetection. Cannot mark it as part of a track."
            )
            if self.misdetLock:
                self._onMarkMisdetectionPressed()
            return -1
    ...
    def _onDivEventPressed(self):
        if self._getActiveTrack() == self.misdetIdx:
            self._criticalMessage(
                "Error: Cannot add a division event for misdetections. Release misdetection button first."
            )
            return
    ...
    def _setStyleSheet(self, widget, qcolor, qType="QComboBox"):
        values = "{r}, {g}, {b}, {a}".format(r=qcolor.red(), g=qcolor.green(), b=qcolor.blue(), a=qcolor.alpha())
            ...
            for t in sorted(oid2tids.keys()):
                ...
                if (oid_prev is not None) and (oid_cur is None):  # track ends
                        ...
                        if t == t_div + 1:
                            ...
                            if (oid_child1 is not None) and (oid_child2 is not None):
                                ...
                                if child1_exists and child2_exists:
                                    self._appendUnique(divs[t], (oid_prev, oid_child1, oid_child2, 0.0))
                                elif child1_exists:
                                    self._appendUnique(moves[t], (oid_prev, oid_child1, 0.0))
                                elif child2_exists:
                                    self._appendUnique(moves[t], (oid_prev, oid_child2, 0.0))
                                break

                    # else: disappearance
                    self._appendUnique(disapps[t], (oid_prev, 0.0))
                    ...
                    if tid in list(tracks_starting_in_div.keys()) and tracks_starting_in_div[tid] != t:
                        self._appendUnique(apps[t], (oid_cur, 0.0))

                elif (oid_prev is not None) and (oid_cur is not None):  # move
                    self._appendUnique(moves[t], (oid_prev, oid_cur, 0.0))
                    ...
                    if len(oid2tids[t][oid_cur]) == 1 and len(oid2tids[t - 1][oid_prev]) > 1:
                        ...
                        if t_multiprev is not None and oid_multiprev is not None:
                            self._appendUnique(multiMoves[t], (oid_multiprev, oid_cur, t_multiprev, 0.0))
        ...
        for t in list(oid2tids.keys()):
            for oid in list(oid2tids[t].keys()):
                if len(oid2tids[t][oid]) > 1:
                    mergers[t].append((oid, len(oid2tids[t][oid]), 0.0))
    ...
    def _onExportDivisionsButtonPressed(self):
        ...
        out_fn, _filter = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save Mergers", os.path.expanduser("~") + "/divisions.csv", options=options
        )
    ...
    def _onExportMergersButtonPressed(self):
        ...
        out_fn, _filter = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save Mergers", os.path.expanduser("~") + "/mergers.csv", options=options
        )
    ...
    def _onExportButtonPressed(self):
        ...
        directory = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select Directory", os.path.expanduser("~"), options=options
        )
        ...
        def _export():
            ...
            for t in sorted(oid2tids.keys()):
                ...
                roi = SubRegion(
                    self.mainOperator.LabelImage,
                    start=[t, 0, 0, 0, 0],
                    stop=[t + 1] + list(self.mainOperator.LabelImage.meta.shape[1:]),
                )
                ...
                try:
                    with h5py.File(fn, "w-") as f_curr:
                        ...
                        if "objects" in list(f_curr.keys()):
                            del f_curr["objects"]
                        ...
                        if len(app_at):
                            ...
                            ds = tg.create_dataset(
                                "Appearances", data=app_at[:, :-1], dtype=numpy.uint32, compression=1
                            )
                            ds.attrs["Format"] = "cell label appeared in current file"
                            ds = tg.create_dataset(
                                "Appearances-Energy", data=app_at[:, -1], dtype=numpy.double, compression=1
                            )
                            ds.attrs["Format"] = "lower energy -> higher confidence"
                        if len(dis_at):
                            dis_at = numpy.array(sorted(dis_at, key=lambda a_entry: a_entry[0]))[::-1]
                            ds = tg.create_dataset(
                                "Disappearances", data=dis_at[:, :-1], dtype=numpy.uint32, compression=1
                            )
                            ds.attrs["Format"] = "cell label disappeared in current file"
                            ds = tg.create_dataset(
                                "Disappearances-Energy", data=dis_at[:, -1], dtype=numpy.double, compression=1
                            )
                            ds.attrs["Format"] = "lower energy -> higher confidence"
                        if len(mov_at):
                            ...
                            ds = tg.create_dataset(
                                "Moves-Energy", data=mov_at[:, -1], dtype=numpy.double, compression=1
                            )
                            ds.attrs["Format"] = "lower energy -> higher confidence"
                        if len(div_at):
                        ...
                        if len(merger_at):
                            ...
                            ds = tg.create_dataset(
                                "Mergers-Energy", data=merger_at[:, -1], dtype=numpy.double, compression=1
                            )
                            ds.attrs["Format"] = "lower energy -> higher confidence"
                        if len(multiMoves_at):
                            multiMoves_at = numpy.array(sorted(multiMoves_at, key=lambda a_entry: a_entry[0]))[::-1]
                            ds = tg.create_dataset(
                                "MultiFrameMoves", data=multiMoves_at[:, :-1], dtype=numpy.uint32, compression=1
                            )
                            ds.attrs["Format"] = "from (file at t_from), to (current file), t_from"
                            ds = tg.create_dataset(
                                "MultiFrameMoves-Energy", data=multiMoves_at[:, -1], dtype=numpy.double, compression=1
                            )
                            ds.attrs["Format"] = "lower energy -> higher confidence"

                except IOError:
                    self._criticalMessage(
                        "File " + str(fn) + " exists already. Please choose a different folder or delete the file(s)."
                    )
                    return

...
from ilastik.array5d.array5D import Array5D, Slice5D, Shape5D, Image, ScalarImage
from ilastik.features.feature_extractor import FeatureCollection, FeatureData


class Sampler(ScalarImage):
    class ClassList(ScalarImage):
        def raw(self): #FIXME: maybe this should only be spawnable as uint32
            return np.asarray(super().raw().astype(np.uint32)[..., np.newaxis])
    ...
    def sample(self, data:Array5D) -> Tuple[FeatureData, ClassList]:
        ...
        samples = FeatureData.allocate(samples_shape, data.dtype)

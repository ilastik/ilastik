...
class TrackingMamutExportFormatPlugin(TrackingExportFormatPlugin):
    ...
    def checkFilesExist(self, filename):
        ...
        return (
            ...
            or os.path.exists(filename + "_raw.h5")
        )
    ...
    def export(self, filename, hypothesesGraph, pluginExportContext):
                        ...
                        if (np.asarray(features[frame][category][key])).ndim == 2:
                            if key != "Histogram":
                                for column in range((np.asarray(features[frame][category][key])).shape[1]):
                                    builder.addFeatureName(
                                        feature_string + "_" + str(column),
                                        feature_string,
                                        shortname + "_" + str(column),
                                        isInt,
                                    )
                        else:
                                    ...
                                    for j in range((np.asarray(features[frame][category][key])).shape[1]):
                                        try:
                                            ...
                                            if "SquaredDistances" in key:
                                                featureDict[feature_string + "_{}".format(str(j))] = 9999.0
                                            else:
                                                featureDict[feature_string + "_{}".format(str(j))] = 0.0
                                            continue
                                        featureDict[feature_string + "_{}".format(str(j))] = features[frame][category][
                                            key
                                        ][label, j]
                                else:
                                    raise ValueError(
                                        f"Found feature matrix {feature_string} that has a dimensionality of > 2, cannot handle that yet"
                                    )
        ...
        assert (
            "bdvFilepath" in additional_plugin_args
        ), "'bdvFilepath' must be present in 'additionalPluginArgumentsSlot'"

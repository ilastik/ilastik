...
class AppletDrawerToolBox(QToolBox):
    ...
    def _get_visible_index(self, index):
        ...
        if visible_index != -1:
            assert visible_index == super(AppletDrawerToolBox, self).indexOf(
                widget
            ), "visible index doesn't match: {} / {}".format(
                visible_index, super(AppletDrawerToolBox, self).indexOf(widget)
            )

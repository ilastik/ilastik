...
class OpGlobalThreshold(Operator):
    ...
    @staticmethod
    def _globalThreshold(vol, threshold=0.25):
        """
        computes an argmax of the prediction maps (hard segmentation), 
        but ignores values below given threshold
        """
    ...
    def execute(self, slot, subindex, roi, result):
        ...
        result[...,0]  = self._globalThreshold(tmp_data, self.Threshold.value)
...
#         self.FinalOutput.connect(self.Output)
#         # from threading import Lock
#         # self.lock= Lock()
...
#         self._opCache.Input.meta.assignFrom(self._Output.meta)
#         self._opCache.Input.meta.dtype=np.float32
#         self._setBlockShape()
#         self._opCache.Input.setDirty(slice(None))
...
#         # self._globalThreshold(tmp_data, self.Threshold.value)
#         slicing = tuple([slice(x,y) for x,y in zip(lower_bound,upper_bound)])
#         result[...] = tmp_data[slicing]

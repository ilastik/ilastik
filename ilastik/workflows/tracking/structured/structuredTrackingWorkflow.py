...
try:
    ...
    try:
        ...
        try:
            ...
            SOLVER = "DPCT"
            logger.warning(
                "Could not find any learning solver. Tracking will use flow-based solver (DPCT). "
...
class StructuredTrackingWorkflowBase(Workflow):
    ...
    def __init__(self, shell, headless, workflow_cmdline_args, project_creation_args, *args, **kwargs):
        ...
        super(StructuredTrackingWorkflowBase, self).__init__(
        ...
        self.trackingFeatureExtractionApplet = TrackingFeatureExtractionApplet(
            name="Object Feature Computation", workflow=self, interactive=False
        )
        ...
        self.dataExportTrackingApplet = TrackingBaseDataExportApplet(
            self, "Tracking Result Export", default_export_filename=self.default_tracking_export_filename
        )
        ...
        self.batchProcessingApplet = BatchProcessingApplet(
            self, "Batch Processing", self.dataSelectionApplet, self.dataExportTrackingApplet
        )
    ...
    def prepare_lane_for_export(self, lane_index):
        ...
        if self.testFullAnnotations:
            ...
            self.trackingApplet.topLevelOperator[lane_index].insertAnnotationsToHypothesesGraph(
                ...
                if key == "detectionResults":
                    ...
                    for i in range(len(solution[key])):
                        ...
                        for j in range(len(solutionFromAnnotations[key])):
                            if (
                                solution[key][i]["id"] == solutionFromAnnotations[key][j]["id"]
                                and solution[key][i]["value"] == solutionFromAnnotations[key][j]["value"]
                            ):
                                flag = True
                                break
                        detectionFlag &= flag
                    ...
                    for i in range(len(solution[key])):
                        ...
                        for j in range(len(solutionFromAnnotations[key])):
                            if (
                                solution[key][i]["dest"] == solutionFromAnnotations[key][j]["dest"]
                                and solution[key][i]["src"] == solutionFromAnnotations[key][j]["src"]
                            ):
                                if (
                                    solution[key][i]["gap"] == solutionFromAnnotations[key][j]["gap"]
                                    and solution[key][i]["value"] == solutionFromAnnotations[key][j]["value"]
                                ):
                                    flag = True
                                    break
    ...
    def post_process_lane_export(self, lane_index, checkOverwriteFiles=False):
        ...
        if (
            self.dataExportTrackingApplet.topLevelOperator.SelectedExportSource.value
            == OpTrackingBaseDataExport.PluginOnlyName
        ):
    ...
    def handleAppletStateUpdateRequested(self):
        ...
        annotations_ready = (
            features_ready
            and len(opAnnotations.Labels) > 0
        ...
        self._shell.setAppletEnabled(
            ...
            structured_tracking_ready
            and not busy
            and self.dataExportTrackingApplet.topLevelOperator.Inputs[0][0].ready(),
        )

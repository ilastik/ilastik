...
class ManualTrackingWorkflow(Workflow):
    ...
    def __init__(self, shell, headless, workflow_cmdline_args, project_creation_args, *args, **kwargs):
        ...
        if "graph" in kwargs:
            del kwargs["graph"]
        super(ManualTrackingWorkflow, self).__init__(
            shell, headless, workflow_cmdline_args, project_creation_args, graph=graph, *args, **kwargs
        )

        data_instructions = (
            'Use the "Raw Data" tab to load your intensity image(s).\n\n'
            'Use the "Prediction Maps" tab to load your pixel-wise probability image(s).'
        )
        ## Create applets
        self.dataSelectionApplet = DataSelectionApplet(
            self, "Input Data", "Input Data", forceAxisOrder=["txyzc"], instructionText=data_instructions, max_lanes=1
        )
        ...
        self.dataExportApplet = TrackingBaseDataExportApplet(
            self, "Tracking Result Export", default_export_filename=self.default_export_filename
        )
    ...
    def handleAppletStateUpdateRequested(self):
        ...
        tracking_ready = (
            features_ready
            and len(opTracking.Labels) > 0
            and opTracking.Labels.ready()
            and opTracking.TrackImage.ready()
        )

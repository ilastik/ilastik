...
FMT = {
    1: BaseDict(
        HEADER="<IIdII",  # version, ....
        CHUNKHEADER="<dI",
        SUBHEADER="<II",
        TIMESTAMP="d",  # XXX struct.pack('<d',nan) dies
    ),
    3: BaseDict(
        HEADER="<4sIQHHB",  # 'ufmf', version, index location,
    ...
    2: BaseDict(
        HEADER="<4sILHHB",  # 'ufmf', version, index location,
        # max w, max h, raw coding string length
        CHUNKID="<B",  # 0 = keyframe, 1 = points
        KEYFRAME1="<B",  # (type name)
        KEYFRAME2="<cHHd",  # (dtype, width,height,timestamp)
    ...
    4: BaseDict(
        HEADER="<4sIQHHBB",  # 'ufmf', version, index location,
...
def _write_dict(fd, save_dict):
    fd.write(b"d")
    fd.write(bytes(chr(len(list(save_dict.keys()))), "utf-8"))
...
class _UFmfV3LowLevelReader:
    ...
    def _fd_read(self, n_bytes, short_OK=False):
        ...
        if len(buf) != n_bytes:
            if not short_OK:
                raise ShortUFMFFileError(
                    "expected %d bytes, got %d: short file %s?" % (n_bytes, len(buf), self._fd.name)
                )
        return buf
...
class _UFmfV3Indexer:
    ...
    def get_index(self):
        result = {b"frame": self._index[b"frame"]}
        # remove defaultdict and convert to dict
        result[b"keyframe"] = {}
        for keyframe_type, value in self._index[b"keyframe"].items():
            result[b"keyframe"][keyframe_type] = value
        return result
    ...
    def _create_index(self):
        self._index = {b"keyframe": collections.defaultdict(dict), b"frame": dict(timestamp=[], loc=[])}
    ...
    def _index_next_chunk(self):
        ...
        try:
            if chunk_id == KEYFRAME_CHUNK:
                ...
                tmp = self._index[b"keyframe"][keyframe_type]
                if len(tmp) == 0:
                    tmp[b"timestamp"] = []
                    tmp[b"loc"] = []
                tmp[b"timestamp"].append(timestamp)
                tmp[b"loc"].append(loc)
            elif chunk_id == FRAME_CHUNK:
                # read frame chunk
                result = self.r._read_frame_chunk()
                timestamp, regions = result
                tmp = self._index[b"frame"]
                tmp[b"timestamp"].append(timestamp)
                tmp[b"loc"].append(loc)
                ...
                raise ValueError("unexpected byte %d where chunk ID expected" % (chunk_id,))
...
class UfmfV3(UfmfBase):
        ...
        **Optional keyword arguments**
        ...
        if index_location == 0:
            ...
            try:
                ...
                b = bytes(chr(INDEX_DICT_CHUNK), "utf-8")
                self._fd.write(b)
                ...
                if raise_write_errors:
                    ...
                    warnings.warn(
                        "IO error when trying to save .ufmf index "
    ...
    def get_number_of_frames(self):
        ...
        try:
            return len(self._index[b"frame"][b"loc"])
        except KeyError:
            raise
    ...
    def get_bg_image(self):
        ...
        return self._get_keyframe_N(b"frame0", 0)
...
class FlyMovieEmulator:
    def __init__(
        ...
        if isinstance(self._ufmf, UfmfV1):
            self.format = b"MONO8"  # by definition
            self._fno2loc = None
                ...
                if h == 1 and w == 1:
                    ...
                    if self._last_frame.ndim == 3:
                        for ptno in range(npts):
                            self._last_frame[
                                locs[1, ptno] : (locs[1, ptno] + h), locs[0, ptno] : (locs[0, ptno] + w), :
                            ] = im[:, :, :, ptno]
                    else:
                        for ptno in range(npts):
                            self._last_frame[
    ...
    def _fill_timestamps_and_locs(self):
        ...
        if isinstance(self._ufmf, UfmfV3):
            ...
            self._timestamps = index[b"frame"][b"timestamp"]
            return
        ...
        try:
            ...
            if int(os.environ.get("UFMF_FORCE_CACHE", "0")):
                ...
                warnings.warn(
                    "While attempting to open cache in %s: %s "
...
class UfmfSaverV3(UfmfSaverBase):
    ...
    def __init__(
        self, file, coding=b"MONO8", frame0=None, timestamp0=None, max_width=None, max_height=None, xinc_yinc=None
    ):
        ...
        buf = struct.pack(
            FMT[self.version].HEADER, b"ufmf", self.version, 0, self.max_width, self.max_height, len(self.coding)
        )
        self.file.write(buf)
        ...
        if xinc_yinc is None:
            if coding == b"MONO8":
                xinc_yinc = (2, 2)
            elif coding.startswith(b"MONO8:"):
                # Bayer pattern
                xinc_yinc = (2, 2)
        ...
        self._index = {b"keyframe": collections.defaultdict(dict), b"frame": {}}
        if frame0 is not None or timestamp0 is not None:
            self.add_keyframe(b"frame0", frame0, timestamp0)
    ...
    def add_keyframe(self, keyframe_type, image_data, timestamp):
        ...
        tmp[b"loc"].append(loc)
        self.file.write(b)
        self.file.write(memoryview(np_image_data))

    def _add_frame_regions(self, timestamp, regions):
        n_pts = len(regions)
        b = bytes(chr(FRAME_CHUNK), "utf-8") + struct.pack(FMT[self.version].POINTS1, timestamp, n_pts)
        loc = self.file.tell()
        tmp = self._index[b"frame"]
        if len(tmp) == 0:
            tmp[b"timestamp"] = []
    ...
    def close(self):
        ...
        self.file.seek(0)
        buf = struct.pack(
            FMT[self.version].HEADER, b"ufmf", self.version, loc, self.max_width, self.max_height, len(self.coding)
        )
        self.file.write(buf)
        if self._file_opened:
            self.file.close()

...
class OpStreamingHdf5SequenceReaderM(Operator):
    ...
    class InconsistentShape(Exception):
        def __init__(self, fileName, datasetName):
            ...
            super(OpStreamingHdf5SequenceReaderM.InconsistentShape, self).__init__(self.msg)
    ...
    class InconsistentDType(Exception):
        def __init__(self, fileName, datasetName):
            ...
            self.msg = "Cannot stack dataset: {} because its data type differs from the type of the previous " \
                       "datasets".format(fileName + '/' + datasetName)
            super(OpStreamingHdf5SequenceReaderM.InconsistentDType, self).__init__(self.msg)
    ...
    def setupOutputs(self):
        ...
        for external_path, internal_path, stacker_slot in zip(
            ...
            try:
                ...
                if dtype is None:
                    dtype = hdfFile[internal_path].dtype
                    shape = hdfFile[internal_path].shape
                else:
                    if dtype is not hdfFile[internal_path].dtype:
                        raise OpStreamingHdf5SequenceReaderM.InconsistentDType(external_path, internal_path)
                    if shape != hdfFile[internal_path].shape:
                        raise OpStreamingHdf5SequenceReaderM.InconsistentShape(external_path, internal_path)
    ...
    def checkGlobString(globString):
        ...
        Raises:
            OpStreamingHdf5SequenceReaderM.InternalPlaceholderError: Raised if
              an internal placeholder is encountered!
            OpStreamingHdf5SequenceReaderM.NoExternalPlaceholderError: Raised if
              only a single path is supplied and no '*'-placeholder can be
              found. -> OpStreamingHdf5Reader should be used in this case.
            OpStreamingHdf5SequenceReaderM.SameFileError: If multiple volumes
              point to the same file this error is raised.
              -> OpStreamingHdf5SequenceReaderS should be used in this case.
            OpStreamingHdf5SequenceReaderM.WrongFileTypeError: If file-
        ...
        if len(pathComponents) == 1:
            if '*' in pathComponents[0].internalPath:
                raise OpStreamingHdf5SequenceReaderM.InternalPlaceholderError(globString)

...
class OpStackLoader(Operator):
    ...
    def setupOutputs(self):
        data_paths = self.DataPaths.value
        self.fileNameList = [str(dp.file_path) for dp in data_paths]
        ...
        if self.slices_per_file == 1:
            ...
            if sequence_axis == "z":
                axistags: str = vigra.defaultAxistags("ztyxc")
    ...
    def propagateDirty(self, slot, subindex, roi):
        ...
        # Any change to the data paths means our entire output is dirty.
        self.stack.setDirty()

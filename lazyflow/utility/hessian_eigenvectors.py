...
def hessian_eigenvectors(a, sigma, sort=True):
    ...
    if hasattr(sigma, "__len__"):
        assert (
            len(sigma) == a.ndim
        ), "sigma {} doesn't match the dimensionality of the input image, which has shape {}".format(sigma, a.shape)
...
def convert_symmetric_tensor_vector_to_full_tensor_matrix(tensor_vector_image, tensor_ndim, upper_only=False):
    ...
    full_tensor = numpy.ndarray(image_shape + (tensor_ndim, tensor_ndim), dtype=tensor_vector_image.dtype)

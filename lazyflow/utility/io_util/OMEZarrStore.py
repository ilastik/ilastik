...
from lazyflow.utility.io_util.connect_to_zarr import ensure_connection_and_get_store
...
def _fetch_and_validate_ome_zarr_spec(uri: str, sort_uri: Optional[str] = None) -> OME_ZARR_SPEC:
    ...
    store = ensure_connection_and_get_store(uri)
    try:
        with Timer() as timer:
...
class OMEZarrStore(MultiscaleStore):
    ...
    def __init__(self, uri: str, target_scale: Optional[str] = None, single_scale_mode: bool = False):
        ...
        if self._multiscale_spec["version"] == "0.1":
            uncached_store = ensure_connection_and_get_store(self.base_uri, **OME_ZARR_V_0_1_KWARGS)
        else:
            uncached_store = ensure_connection_and_get_store(self.base_uri, **OME_ZARR_V_0_4_KWARGS)

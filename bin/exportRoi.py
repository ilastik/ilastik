...
argparser.add_argument(
    "--format",
    default="blockwise-subset",
    help="The format to export to.",
    choices=["single-hdf5", "blockwise-subset"],
    required=False,
)
argparser.add_argument(
    "--coordinates",
    default="absolute",
    help="How to interpret the roi arg.",
    choices=["absolute", "view"],
    required=False,
)
argparser.add_argument("description_path", help="Path to source dataset description JSON file.")
argparser.add_argument(
    "roi",
    help="The roi you want to export, in ABSOLUTE coordinates (i.e. the coords on the filesystem, not the current view)",
)
argparser.add_argument("export_dir", help="The directory to write the cutout data to.")

example_usage = (
    'Example: %(prog)s --format=blockwise-subset data_description.json "[(0,10,20), (10,20,30)]" /tmp/exported_data\n'
)
...
if not isinstance(roi, (list, tuple)) or not isinstance(roi[0], (list, tuple)) or not isinstance(roi[1], (list, tuple)):

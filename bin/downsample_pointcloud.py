...
def downsample_pointcloud(
    ...
    density_volume_zyx = density_volume_from_pointcloud(
        pointcloud_csv_filepath, scale_xyz, offset_xyz, volume_shape_xyz, method
    )
    ...
    if smoothing_sigma_xyz:
        ...
        vigra.filters.gaussianSmoothing(density_volume_zyx, smoothing_sigma_zyx, out=density_volume_zyx)
...
def density_volume_from_pointcloud(
    ...
    assert expected_columns.issubset(data_columns), (
        ...
        "Expected columns: {},\n"
        "Your file's columns: {}".format(POINTCLOUD_COLUMNS, list(pointcloud_data.dtype.fields.keys()))
    )

    # Determine offset if not provided.
    if not offset_xyz:
        offset_xyz = (pointcloud_data["x_px"].min(), pointcloud_data["y_px"].min(), pointcloud_data["z_px"].min())

    # Apply offset
    logger.debug("Subtracting offset: {}".format(offset_xyz))
    for axis, axis_offset in zip("xyz", offset_xyz):
        fields = ["{}_px".format(axis), "min_{}_px".format(axis), "max_{}_px".format(axis)]
        for field in fields:
            pointcloud_data[field] -= axis_offset

    # Determine volume shape if not provided.
    if not volume_shape_xyz:
        volume_shape_xyz = (pointcloud_data["x_px"].max(), pointcloud_data["y_px"].max(), pointcloud_data["z_px"].max())
    ...
    # All coords
    coordinates_zyx = (pointcloud_data["z_px"], pointcloud_data["y_px"], pointcloud_data["x_px"])
...
def export_hdf5(density_volume_zyx, output_filepath, dset_name):
    ...
    with h5py.File(output_filepath, "w") as f_out:
        dset = f_out.create_dataset(dset_name, density_volume_zyx.shape, density_volume_zyx.dtype, chunks=True)
...
def export_tiff(density_volume_zyx, output_filepath):
    ...
    """
    assert os.path.splitext(output_filepath)[1] in [".tif", ".tiff"], "Wrong extension for TIFF export: {}".format(
        output_filepath
    )
    import vigra
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--method",
        choices=["binary", "by_count", "by_size"],
        default="by_count",
    ...
    sys.exit(
        downsample_pointcloud(
            parsed_args.pointcloud_csv_filepath,

...
The Axisiterator has been designed to provide a way to iterate through two volumes which is easy on the eye. An example of a typical use is:
...
As you can see, for every ``trgt`` slice there is a corresponding ``src`` and ``mask`` slice. In the first step the ``roi`` gets broken down in a number of ``trgt`` slices:
...
This operator mimics the natural behavior of the ``vigra.filters.structureTensorEigenvalues`` filter. The operator class is a subclass of ``OpBaseVigraFilter``. While ``OpBaseVigraFilter`` holds the general functionality for filter operations, the subclasses implement the individual filter features. 

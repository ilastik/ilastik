...
class TestPixelClassificationSerializer(object):
    def test(self):
        ...
        with h5py.File(testProjectName, "a") as testProject:
            ...
            assert operatorToSave.LabelNames.value == operatorToLoad.LabelNames.value, "{} != {}".format(
                operatorToSave.LabelNames.value, operatorToLoad.LabelNames.value
            )
            assert (
                numpy.array(operatorToSave.LabelColors.value) == numpy.array(operatorToLoad.LabelColors.value)
            ).all()

...
from lazyflow.classifiers.parallelVigraRfLazyflowClassifier import ParallelVigraRfLazyflowClassifierFactory, ParallelVigraRfLazyflowClassifier
...
class TestIlastikParser:
    @pytest.mark.parametrize("proj, expected_num_channels", [
    ...
    @pytest.mark.parametrize("proj, expected_factory, expected_classifier", [
        (TestData.PIXEL_CLASS_1_CHANNEL, ParallelVigraRfLazyflowClassifierFactory, ParallelVigraRfLazyflowClassifier),
    ...
    tests = [
        (TestData.PIXEL_CLASS_1_CHANNEL, np.array([
        ...
        (TestData.PIXEL_CLASS_3_CHANNEL, np.array([
    ...
    @pytest.mark.parametrize("proj, expected_feature_matrix", tests)
    def test_parse_project_features(self, test_data_lookup, proj, expected_feature_matrix):
        project_path = test_data_lookup.find(proj)
        ...
        with IlastikProject(project_path) as proj:
            ...
            print("EXPECTED", expected_feature_matrix)
            np.testing.assert_array_equal(matrix.matrix, expected_feature_matrix)

...
class TestTrackingBaseDataExportAppletSerialization(object):
    ...
    def testAppletSerialization(self):
        ...
        with h5py.File(self.TEST_PROJECT_FILE) as testProject:
            ...
            assert (
                testProject["Tracking Result Export/AdditionalPluginArguments/bdvFilePath"].value.decode()
                == "/tmp/bdv.xml"
            )
    ...
    def testAppletDeserialization(self):
        ...
        with h5py.File(self.TEST_PROJECT_FILE) as testProject:
            ...
            assert (
                dataExportSerializer.topLevelOperator.AdditionalPluginArguments.value["bdvFilePath"] == "/tmp/bdv.xml"
            )

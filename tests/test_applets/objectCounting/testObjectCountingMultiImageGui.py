...
class TestObjectCountingGuiMultiImage(ShellGuiTestCaseBase):
    ...
    def test_9_CheckDensity(self):
        ...
        def impl():
            ...
            assert (
                abs(displayedDensity - operatorDensity) < 1e-1
            ), "Density mismatch:, the displayed Density {} is not\
            equal to the internal density from the Operator {}".format(
                displayedDensity, operatorDensity
            )

            assert (
                abs(operatorDensity - sumDensity) < 1e-1
            ), "Density mismatch: the Sum operator {} does not return the same\
            result as using numpy.sum {}".format(
                operatorDensity, sumDensity
            )
    ...
    def test_11_CheckBoxes(self):
        ...
        def impl():
            ...
            assert (
                abs(fullBoxVal - fullBoxVal2) < 1e-5
            ), "Box mismatch: {.2f} is not close to\
            {.2f}".format(
                fullBoxVal, fullBoxVal2
            )

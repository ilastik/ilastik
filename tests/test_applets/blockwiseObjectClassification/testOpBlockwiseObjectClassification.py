...
class TestOpBlockwiseObjectClassification(unittest.TestCase):
    ...
    def testPredictionChannelImage(self):
        ...
        if not (argmax_pred == self.prediction_volume).all():
            ...
            assert False, "Blockwise prediction channels did not correspond to the expected prediction image"
    ...
    def setUpSources(self):
        ...
        self.testingFeatures = {
            "Standard Object Features": {"Count": {}, "Mean": {}, "Mean in neighborhood": {"margin": (10, 10, 1)}}
        }
    ...
    def setUpClassifier(self):
        ...
        coordE = (0, 60, 20, 20, 0)
        assert (
            prediction_volume[coordE] == 1
        ), "Expected object #{} to be predicted as class 1, but got prediction {}".format(
            object_volume[coordE], prediction_volume[coordE]
        )
        coordF = (0, 10, 30, 30, 0)
        assert (
            prediction_volume[coordF] == 1
        ), "Expected object #{} to be predicted as class 1, but got prediction {}".format(
        ...
        coordG = (0, 70, 30, 30, 0)
        assert (
            prediction_volume[coordG] == 2
        ), "Expected object #{} to be predicted as class 2, but got prediction {}".format(
            object_volume[coordG], prediction_volume[coordG]
        )
        coordH = (0, 20, 20, 20, 0)
        assert (
            prediction_volume[coordH] == 2
        ), "Expected object #{} to be predicted as class 2, but got prediction {}".format(

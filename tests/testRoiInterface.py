...
class TestRoiUtilities(TestCase):
    ...
    def test_sliceToRoi(self):
        ...
        # Single slice
        assert lazyflow.roi.sliceToRoi(slice(None), shape) == (
            TinyVector((0, 0, 0, 0, 0)),
            TinyVector((2, 4, 6, 8, 10)),
        )
        assert lazyflow.roi.sliceToRoi(slice(0, 1), shape) == (
            TinyVector((0, 0, 0, 0, 0)),
            TinyVector((1, 4, 6, 8, 10)),
        )
        ...
        assert lazyflow.roi.sliceToRoi([slice(0, 1), 3, 4], shape) == (
            TinyVector((0, 3, 4, 0, 0)),
            TinyVector((1, 4, 5, 8, 10)),
        )
        ...
        # Tuple/list with Ellipsis
        assert lazyflow.roi.sliceToRoi((0, 1, Ellipsis, 9), shape) == (
            TinyVector((0, 1, 0, 0, 9)),
            TinyVector((1, 2, 6, 8, 10)),
        )
        assert lazyflow.roi.sliceToRoi([0, 1, Ellipsis, 9], shape) == (
            TinyVector((0, 1, 0, 0, 9)),
            TinyVector((1, 2, 6, 8, 10)),
        ...
        # Single index
        assert lazyflow.roi.sliceToRoi(1, shape, extendSingleton=False) == (
            TinyVector((1, 0, 0, 0, 0)),
            TinyVector((1, 4, 6, 8, 10)),
        )

        # Single slice
        assert lazyflow.roi.sliceToRoi(slice(None), shape, extendSingleton=False) == (
            TinyVector((0, 0, 0, 0, 0)),
            TinyVector((2, 4, 6, 8, 10)),
        )
        assert lazyflow.roi.sliceToRoi(slice(0, 1), shape, extendSingleton=False) == (
            TinyVector((0, 0, 0, 0, 0)),
            TinyVector((1, 4, 6, 8, 10)),
        )
        ...
        assert lazyflow.roi.sliceToRoi([slice(0, 1), 3, 4], shape, extendSingleton=False) == (
            TinyVector((0, 3, 4, 0, 0)),
            TinyVector((1, 3, 4, 8, 10)),
        )
        ...
        # Single Ellipsis
        assert lazyflow.roi.sliceToRoi(Ellipsis, shape, extendSingleton=False) == (
            TinyVector((0, 0, 0, 0, 0)),
            TinyVector((2, 4, 6, 8, 10)),
        )

        # Tuple/list with Ellipsis
        assert lazyflow.roi.sliceToRoi((0, 1, Ellipsis, 9), shape, extendSingleton=False) == (
            TinyVector((0, 1, 0, 0, 9)),
            TinyVector((0, 1, 6, 8, 9)),
        )
        assert lazyflow.roi.sliceToRoi([0, 1, Ellipsis, 9], shape, extendSingleton=False) == (
            TinyVector((0, 1, 0, 0, 9)),
            TinyVector((0, 1, 6, 8, 9)),
    ...
    def test_roiToSlice(self):
        ...
        assert lazyflow.roi.roiToSlice(roi[0], roi[1]) == (
            slice(1, 2),
            slice(2, 4),
            slice(3, 6),
            slice(4, 8),
